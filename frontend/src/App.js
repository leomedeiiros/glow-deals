...
function App() {
 ...
 const [titlePrompt, setTitlePrompt] = useState(''); // MUDANÇA: Campo vazio por padrão
...
// Handler para cupom de desconto
 ...
 const handleGenerateTitle = async () => {
   ...
   if (!titlePrompt.trim()) {
     setError('Por favor, escreva um prompt para gerar o título.');
     return;
   }
   ...
   try {
     ...
     const response = await axios.post('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent', {
       contents: [{ parts: [{ text: `${titlePrompt}. Produto: ${productData.name}` }] }],
       generationConfig: {
         temperature: 0.9,
         maxOutputTokens: 50
       }
   ...
   <div className="container">
     ...
     <div className="main-card" ref={mainCardRef}>
       ...
       {aiImageSectionOpen && (
         <div className="section-content">
           <div className="form-group">
             <label className="form-label">Prompt para o Título</label>
             <textarea 
               className="form-input"
               style={{ minHeight: '60px' }}
               value={titlePrompt}
               onChange={(e) => setTitlePrompt(e.target.value)}
               placeholder="Ex: Gere um título divertido em MAIÚSCULAS de até 50 caracteres para uma promoção no WhatsApp"
           ...
           <button 
             ...
             disabled={generatingTitle || !productData || !titlePrompt.trim()}
           >
             {generatingTitle ? (
               <>
                 <div className="loading"></div>
